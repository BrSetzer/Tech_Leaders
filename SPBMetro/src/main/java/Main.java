...
public class Main {
    ...
    private static List exception; // неициализир. переменная, чтобы выбросить ошибку для проверки работы логгера
    ...
    public static void main(String[] args) {
        ...
        try {
            for (; ; ) {
                ...
                printRoute(route);
                // System.out.println(exception.get(0));
                System.out.println("Длительность: " +
                        RouteCalculator.calculateDuration(route) + " минут");
            }
        } catch (Exception e) {
            LOGGER.error(EXCEPTION_MARKER, "Error close session {}", e);
    ...
    private static RouteCalculator getRouteCalculator() {
        createStationIndex(); // объект для хранения данных карты метро
    ...
    private static Station takeStation(String message) {
        for (; ; ) {
            ...
            LOGGER.warn(INVALID_STATIONS_MARKER, "Нет такой станции: {} ", line);
